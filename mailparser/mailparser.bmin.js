(function e(t, n, r) {
  ...
  1: [function(require, module, exports) {
    MailParser = require("../node_modules/mailparser/lib/mailparser").MailParser
  }, {
    "../node_modules/mailparser/lib/mailparser": 3
  }],
  ...
  3: [function(require, module, exports) {
    (function(process, Buffer) {
      ...
      MailParser.prototype._processStateHeader = function(line) {
        ...
        if (!line.length) {
          ...
          if (this._currentNode.attachment) {
            ...
            this._currentNode.meta.contentId = this._currentNode.meta.contentId || crypto.createHash("md5").update(this._currentNode.meta.generatedFileName).digest("hex") + "@mailparser";
      ...
      MailParser.prototype._processStateBody = function(line) {
        var i, len, node, nodeReady = false;
        if (line.substr(0, 2) == "--") {
          for (i = 0, len = this._multipartTree.length; i < len; i++) {
            if (line == "--" + this._multipartTree[i].boundary) {
              ...
              nodeReady = true;
              break
            } else if (line == "--" + this._multipartTree[i].boundary + "--") {
              if (this._currentNode.content || this._currentNode.stream) {
                this._finalizeContents()
              }
        ...
        if (["text/plain", "text/html", "text/calendar"].indexOf(this._currentNode.meta.contentType || "") >= 0 && !this._currentNode.attachment) {
        ...
        switch (key) {
          ...
          case "date":
            ...
            if (Object.prototype.toString.call(this._currentNode.meta.date) != "[object Date]" || this._currentNode.meta.date.toString() == "Invalid Date") {
              this._currentNode.meta.date = datetime.strtotime(value) && new Date(datetime.strtotime(value) * 1e3)
            }
            break;
          case "to":
      ...
      MailParser.prototype._parseReferences = function(value) {
        this._currentNode.references = (this._currentNode.references || []).concat((value || "").toString().trim().split(/\s+/).map(this._trimQuotes.bind(this)))
      };
      MailParser.prototype._parseInReplyTo = function(value) {
        this._currentNode.inReplyTo = (this._currentNode.inReplyTo || []).concat((value || "").toString().trim().split(/\s+/).map(this._trimQuotes.bind(this)))
      };
      ...
      MailParser.prototype._joinHTMLNodes = function(htmlNode, newHTML) {
        ...
        newHTML = newHTML.replace(/<head( [^>]*)?>(.*)<\/head( [^>]*)?>/gi, "").replace(/<\/?html( [^>]*)?>/gi, "").trim();
      ...
      MailParser.prototype._concatHTML = function(firstNode, secondNode) {
        ...
        if (headerNode) {
          headerNode.replace(/\r?\n/g, "\x00").replace(/^[\s\u0000]*(<\!doctype\b[^>]*?>)?[\s\u0000]*<(html|head)\b[^>]*>.*?<\/(head)\b[^>]*>(.*?<body\b[^>]*>)?/i, function(h) {
            ...
            h = h.replace(/<[\!\/]?(doctype|html|head|body)\b[^>]*?>/gi, "\x00").replace(/\u0000+/g, "\n").trim();
      ...
      MailParser.prototype._replaceMimeWords = function(value) {
        return value.replace(/(=\?[^?]+\?[QqBb]\?[^?]*\?=)\s+(?==\?[^?]+\?[QqBb]\?[^?]*\?=)/g, "$1").replace(/\=\?[^?]+\?[QqBb]\?[^?]*\?=/g, function(a) {
          return mimelib.decodeMimeWord(a.replace(/\s/g, ""))
        }.bind(this))
      };
      MailParser.prototype._trimQuotes = function(value) {
        value = (value || "").trim();
        if (value.charAt(0) == '"' && value.charAt(value.length - 1) == '"' || value.charAt(0) == "'" && value.charAt(value.length - 1) == "'" || value.charAt(0) == "<" && value.charAt(value.length - 1) == ">") {
      ...
      MailParser.prototype._updateHTMLCharset = function(html) {
        html = html.replace(/\n/g, "\x00").replace(/<meta[^>]*>/gi, function(meta) {
  ...
  4: [function(require, module, exports) {
    (function(Buffer) {
      ...
      QPStream.prototype.flush = function() {
        ...
        if (this.charset.toLowerCase() == "binary") {} else if (this.charset.toLowerCase() != "utf-8") {
  ...
  6: [function(require, module, exports) {
    (function(Buffer) {
      ...
      function DBCSCodec(options) {
        ...
        if (!options.table) throw new Error("Encoding '" + options.encodingName + "' has no data.");
        ...
        for (var uChar in options.encodeAdd || {}) this._setEncodeChar(uChar.charCodeAt(0), options.encodeAdd[uChar]);
      ...
      DBCSCodec.prototype._getDecodeTrieNode = function(addr) {
        ...
        for (var i = bytes.length - 1; i > 0; i--) {
          ...
          } else throw new Error("Overwrite byte in " + this.options.encodingName + ", addr: " + addr.toString(16))
        }
        return node
      };
      DBCSCodec.prototype._addDecodeChunk = function(chunk) {
        ...
        for (var k = 1; k < chunk.length; k++) {
          ...
          if (typeof part === "string") {
            for (var l = 0; l < part.length;) {
              ...
              if (55296 <= code && code < 56320) {
                ...
                else throw new Error("Incorrect surrogate pair in " + this.options.encodingName + " at chunk " + chunk[0])
          ...
          } else throw new Error("Incorrect type '" + typeof part + "' given in " + this.options.encodingName + " at chunk " + chunk[0])
        }
        if (curAddr > 255) throw new Error("Incorrect chunk in " + this.options.encodingName + " at addr " + chunk[0] + ": too long" + curAddr)
      };
  ...
  7: [function(require, module, exports) {
    ...
    "./tables/big5-added.json": 13,
    "./tables/cp936.json": 14,
    "./tables/cp949.json": 15,
    "./tables/cp950.json": 16,
    "./tables/eucjp.json": 17,
    "./tables/gb18030-ranges.json": 18,
    "./tables/gbk-added.json": 19,
    "./tables/shiftjis.json": 20
  }],
  8: [function(require, module, exports) {
    ...
    for (var i = 0; i < modules.length; i++) {
      ...
      for (var enc in module) exports[enc] = module[enc]
    }
  }, {
    "./dbcs-codec": 6,
    "./dbcs-data": 7,
    ...
    "./sbcs-codec": 10,
    "./sbcs-data": 12,
    "./sbcs-data-generated": 11,
    "./utf16": 21,
    "./utf7": 22
  }],
  9: [function(require, module, exports) {
    (function(Buffer) {
      var utf16lebom = new Buffer([255, 254]);
      module.exports = {
        utf8: {
          type: "_internal",
          enc: "utf8"
        ...
        _internal: function(options) {
          if (!options || !options.enc) throw new Error("Internal codec is called without encoding type.");
          return {
  ...
  10: [function(require, module, exports) {
    (function(Buffer) {
      exports._sbcs = function(options) {
        ...
        for (var i = 0; i < options.chars.length; i++) encodeBuf[options.chars.charCodeAt(i)] = i;
        return {
          encoder: encoderSBCS,
          decoder: decoderSBCS,
          encodeBuf: encodeBuf,
  ...
  21: [function(require, module, exports) {
    (function(Buffer) {
      ...
      function utf16Encoder(options) {
        ...
        if (!codec.bom) throw new Error("iconv-lite: in UTF-16 encoder, 'use' parameter should be either UTF-16BE or UTF16-LE.");
        return {
      ...
      function utf16DecoderDecideEndianness() {
        ...
        if (buf[0] == 254 && buf[1] == 255) {
          ...
          for (var i = 0; i < _len; i += 2) {
            if (buf[i] == 0 && buf[i + 1] == 32) spaces[0] ++;
            if (buf[i] == 32 && buf[i + 1] == 0) spaces[1] ++
          }
  ...
  25: [function(require, module, exports) {
    ...
    "./lib/content-types": 27,
    "./lib/content-types-reversed": 26,
    "./lib/mimelib": 28
  }],
    ...
    (function(Buffer) {
      ...
      module.exports.mimeFunctions = {
        mimeEncode: function(str, toCharset, fromCharset) {
          ...
          for (var i = 0, len = buffer.length; i < len; i++) {
            ...
            result += "=" + (buffer[i] < 16 ? "0" : "") + buffer[i].toString(16).toUpperCase()
          }
          return result
        },
        mimeDecode: function(str, toCharset, fromCharset) {
          ...
          var encodedBytesCount = (str.match(/\=[\da-fA-F]{2}/g) || []).length,
            ...
            chr, hex, buffer = new Buffer(bufferLength),
            bufferPos = 0;
      ...
      function addQPSoftLinebreaks(mimeEncodedStr, lineLengthMax) {
        var pos = 0,
          ...
          match, code, line, lineMargin = Math.floor(lineLengthMax / 3),
          result = "";
        while (pos < len) {
    ...
    (function(Buffer) {
      ...
      UUE.prototype.encode = function(encodeSource, encodeOptions) {
        ...
        if (typeof encodeSource === "string") {
          if (typeof encodeOptions.mode === "undefined") {} else if (typeof encodeOptions.mode !== "string") {
      ...
      UUE.prototype.decodeFile = function(text, filename) {
        var matches = [];
        var potentialUUE = RegExp(["^begin [0-7]{3} " + filename + "\n", "(", "(?:[ -`]+\n)*", ")", "`\n", "end$"].join(""), "gm");
  ...
  41: [function(require, module, exports) {
    ...
    "./prng": 176,
    "./rng": 177,
    "browserify-aes/inject": 50,
    "browserify-sign/algos": 61,
    "browserify-sign/inject": 62,
    "create-ecdh/inject": 110,
    "diffie-hellman/inject": 135,
    "public-encrypt/inject": 141
  }],
